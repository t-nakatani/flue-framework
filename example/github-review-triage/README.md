...
この example は issue から実装 branch と PR を作るため、`Contents: write` と `Pull requests: write` が必要です。PR への summary comment は issue comment API で投稿します。GitHub では PR も issue として comment できるため、PR summary comment には `Issues: write` が必要です。

implementation status label は事前に repository に作成しておくのが安全です。agent 側で label を自動作成する運用にする場合は、token に label 作成が可能な `Issues: write` 権限が必要です。
...
Worker は dispatch 前に現在の issue labels を再取得し、`agent:in-progress` を先に付けてから agent を起動します。これにより、通常の webhook redelivery や後続の `labeled` event による重複起動を抑えます。初回利用で status label が未作成の場合は予約だけをスキップし、agent 側の label 作成に任せます。
